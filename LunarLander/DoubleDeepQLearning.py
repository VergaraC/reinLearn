...
class DoubleDeepQLearning:
    ...
    def __init__(self, env, gamma, epsilon, epsilon_min, epsilon_dec, episodes, batch_size, memory, model):
        ...
        self.memory = memory
        self.model = model # primary updatado toda vez
        self.model2 = tf.keras.models.clone_model(model) # target updatado a cada alguns episodios que pega
        self.model2.set_weights(self.model.get_weights())

    def select_action(self, state):
        if np.random.rand() < self.epsilon:
            return random.randrange(self.env.action_space.n)
        action = self.model2.predict(state)
    ...
    def experience_replay(self, episodes):
        if len(self.memory) > self.batch_size:
            ...
            next_max = np.amax(self.model2.predict_on_batch(next_states), axis=1)
